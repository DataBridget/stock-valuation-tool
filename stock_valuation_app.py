...
def calculate_forward_pe_valuation(eps, growth_rate, pe_forward):
    """远期PE估值：用预测EPS × 远期PE"""
    if eps <= 0 or growth_rate <= 0:
        return 0
    forward_eps = eps * (1 + growth_rate)
    return forward_eps * pe_forward

def calculate_peg_valuation(eps, growth_rate, peg_ratio=1.0):
    """PEG估值：PE = 增长率 × PEG，股价 = EPS × PE"""
    if eps <= 0 or growth_rate <= 0:
        return 0
    pe_from_peg = growth_rate * 100 * peg_ratio
    return eps * pe_from_peg
...
def main():
    ...
    def recommend_params(name, pe_ttm, pb_mrq, roe_val, yoy_np_val):
        ...
        if any(k in name for k in ['银行', '招商', '平安', '工商', '建设', '农业', '中国', '兴业', '浦发', '民生', '光大', '华夏', '北京', '上海', '江苏', '南京', '宁波', '杭州', '成都', '长沙', '苏州', '无锡', '常熟', '江阴', '张家港', '吴江', '昆山', '太仓']):
            return "银行股", 5, 8, 12, 0.05, 0.08, 0.03, 1.2, "PB为主", 0.1, 0.1, 0.8, 8, 1.0
        # 保险/券商/信托
        elif any(k in name for k in ['保险', '证券', '券商', '信托', '期货', '新华', '人寿', '太保', '平安']):
            return "金融股", 8, 12, 18, 0.08, 0.09, 0.03, 1.5, "PB为主", 0.2, 0.2, 0.6, 12, 1.0
        # 地产股
            ...
            return "地产股", 6, 10, 15, 0.05, 0.10, 0.02, 1.0, "PB为主", 0.1, 0.1, 0.8, 10, 1.0
        # 煤炭/钢铁/有色/石油（周期股）
            ...
            return "周期股", 8, 12, 20, 0.08, 0.10, 0.02, 1.5, "PE+PB", 0.3, 0.2, 0.5, 15, 1.0
        # 白酒/食品饮料（消费白马）
            ...
            return "消费白马", 20, 30, 45, 0.15, 0.09, 0.03, 4.0, "PE为主", 0.6, 0.2, 0.2, 30, 1.2
        # 医药
            ...
            return "医药股", 25, 35, 55, 0.18, 0.09, 0.03, 4.5, "PE为主", 0.6, 0.2, 0.2, 35, 1.2
        # 光通信/CPO/算力（AI硬件，参照券商研报PE 30-45x）
            ...
            return "光通信/CPO", 25, 35, 50, 0.40, 0.10, 0.03, 8.0, "PE为主", 0.6, 0.2, 0.2, 35, 1.5
        # 新能源/光伏/锂电
            ...
            return "新能源", 30, 50, 80, 0.25, 0.10, 0.04, 5.0, "PE为主", 0.6, 0.2, 0.2, 50, 1.5
        # 半导体/芯片/AI/科技
            ...
            return "科技股", 35, 60, 100, 0.30, 0.10, 0.04, 6.0, "PE为主", 0.6, 0.2, 0.2, 60, 1.5
        # 高成长型（通用）
        elif pe_ttm > 50 or (yoy_np_val and yoy_np_val > 30):
            return "高成长型", 30, 50, 80, 0.25, 0.10, 0.04, 5.0, "PE为主", 0.5, 0.3, 0.2, 50, 1.5
        # 价值型
        elif pe_ttm < 15 and roe_val > 15:
            return "价值型", 10, 15, 25, 0.08, 0.08, 0.03, 2.0, "PE+PB", 0.4, 0.2, 0.4, 15, 1.0
        # 稳健型
        elif pe_ttm < 25 and (yoy_np_val and yoy_np_val < 10):
            return "稳健型", 12, 20, 35, 0.10, 0.09, 0.03, 2.5, "PE为主", 0.5, 0.2, 0.3, 20, 1.0
        else:
            return "成长型", 15, 30, 50, 0.15, 0.10, 0.03, 3.0, "PE为主", 0.5, 0.2, 0.3, 30, 1.2

    stock_type, rec_pe_low, rec_pe_mid, rec_pe_high, rec_growth, rec_discount, rec_terminal, rec_pb, val_method, w_pe, w_dcf, w_pb, rec_forward_pe, rec_peg = \
    ...
    if use_recommended:
        ...
        forward_pe, peg_ratio = rec_forward_pe, rec_peg
        ...
        with st.sidebar.expander("远期估值参数"):
            forward_pe = st.sidebar.slider("远期PE", 5, 100, rec_forward_pe)
            peg_ratio = st.sidebar.slider("PEG比率", 0.5, 3.0, rec_peg, step=0.1)
    ...
    forward_pe_val = calculate_forward_pe_valuation(eps, growth, forward_pe)
    peg_val = calculate_peg_valuation(eps, growth, peg_ratio)
    # 高成长股票：远期估值权重更高
    if stock_type in ['光通信/CPO', '新能源', '科技股', '高成长型'] or growth > 0.30:
        fair_price = pe_base * 0.3 + dcf_val * 0.2 + pb_val * 0.1 + forward_pe_val * 0.25 + peg_val * 0.15
    else:
        fair_price = pe_base * w_pe + dcf_val * w_dcf + pb_val * w_pb

    valuation = {'pe': (pe_pessimistic, pe_base, pe_optimistic), 'dcf': dcf_val, 'pb': pb_val, 'forward_pe': forward_pe_val, 'peg': peg_val, 'fair': fair_price}
    ...
    fwd_col1.metric("远期PE估值", f"¥{forward_pe_val:.2f}", f"远期PE={forward_pe}x")
    fwd_col2.metric("PEG估值", f"¥{peg_val:.2f}", f"PEG={peg_ratio}")
    fwd_col3.metric("估值方法", "远期加权" if stock_type in ['光通信/CPO', '新能源', '科技股', '高成长型'] or growth > 0.30 else "标准加权", stock_type)
